...
class One_And_One_Plugin_Manager {
	...
	public function get_plugin_manager( $site_type, $theme_id ) {
		...
		$plugins_json = One_And_One_Sitetype_Filter::get_plugins_by_sitetype( $site_type );
		...
		$recommended_plugins = One_And_One_Sitetype_Filter::get_filtered_plugins_by_configkey( $plugins, $site_type, 'recommended_plugins' );
		$more_plugins        = One_And_One_Sitetype_Filter::get_filtered_plugins_by_configkey( $plugins, $site_type, 'more_plugins' );
			...
			<div class="wrap">
				...
				<div class="oneandone-functionality-choice">
					...
					if ( $recommended_plugins ) {
						...
						$subNaviSections['recommended_plugins'] = strip_tags( One_And_One_Sitetype_Filter::get_headline_by_configkey( $site_type, 'recommended_plugins' ) );
						?>

						<a name="recommended_plugins" id="recommended_plugins"></a>
						<?php echo One_And_One_Sitetype_Filter::get_headline_by_configkey( $site_type, 'recommended_plugins' ); ?>
						<?php echo One_And_One_Sitetype_Filter::get_subline_by_configkey( $site_type, 'recommended_plugins' ); ?>
					...
					if ( $more_plugins ) {
						...
						$subNaviSections['more_plugins'] = strip_tags( One_And_One_Sitetype_Filter::get_headline_by_configkey( $site_type, 'more_plugins' ) );
						?>
						c
						<a name="more_plugins" id="more_plugins"></a>
						<?php echo One_And_One_Sitetype_Filter::get_headline_by_configkey( $site_type, 'more_plugins' ); ?>
						<?php echo One_And_One_Sitetype_Filter::get_subline_by_configkey( $site_type, 'more_plugins' ); ?>

						<div class="oneandone-plugin-browser">
							<?php
							foreach ( $more_plugins as $plugin ) {
								$popup_id = "Popup".$index ++;
					...
					<?php if ( $category_plugins ) {
						...
						foreach ( $category_plugins as $category ) {
							$count ++;
							...
							if ( $category['headline'] ): ?>
								...
								<?php $headLine = _x( $category['headline'], 'json_plugins_section', '1and1-wordpress-wizard' ); ?>
								<?php
								$catKey                   = 'cat'.$category['headline'];
								$subNaviSections[$catKey] = strip_tags( $headLine );
								?>
								<a name="<?php echo $catKey; ?>" id="<?php echo $catKey; ?>"></a>
								<h3><?php print $headLine; ?></h3>
							<?php endif; ?>

							<?php if ( $category['subline'] ): ?>
								<p><?php _ex( $category['subline'], 'json_plugins_section', '1and1-wordpress-wizard' ); ?></p>
							<?php endif; ?>
	...
	public function get_installed_plugin_slugs() {
		if ( empty( $this->slugs ) ) {
			...
			$plugins_meta = One_And_One_Sitetype_Filter::get_plugins_by_sitetype( $this->site_type );
